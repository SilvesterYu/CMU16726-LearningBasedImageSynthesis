...
def save_samples(iteration, fixed_Y, fixed_X, G_YtoX, G_XtoY, opts):
    ...
    print("fake Y shape", fake_Y.shape)
...
def training_loop(dataloader_X, dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters + 1):
        ...
        if opts.use_cycle_consistency_loss:
            # print("images_Y", images_Y.shape)
            # print("fake_X", fake_X.shape)
            # print("fake X to Y", G_XtoY(fake_X).shape)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = torch.nn.functional.l1_loss(G_XtoY(fake_X), images_Y)
            # torch.norm(torch.absolute(images_Y - G_XtoY(fake_X)))
        ...
        if opts.use_cycle_consistency_loss:
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = torch.nn.functional.l1_loss(G_YtoX(fake_Y), images_X)
            # torch.norm(torch.absolute(images_X - G_YtoX(fake_X)))
